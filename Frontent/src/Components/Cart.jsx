...
function Cart() {
  ...
  // Helper for safe quantity lookup
  const safeQuantity = (id) => (Quantity && Quantity[id]) || 1;
  ...
  const fetchData = useCallback(async (cartIds = null, silent = false) => {
    ...
    try {
      ...
      filteredResults.forEach((item) => {
        newQuantities[item._id] = safeQuantity(item._id);
      });
      if (isMounted.current) {
  ...
  useEffect(() => {
    const handleCartUpdate = () => { refreshCart(true); };
    const handleStorageChange = (e) => {
      if (e.key === 'cartUpdated') { refreshCart(true); }
    };
    const handleVisibilityChange = () => {
      if (!document.hidden) { refreshCart(true); }
    };
    const handleFocus = () => { refreshCart(true); };
  ...
  useEffect(() => {
    let total = 0;
    data.forEach((item) => {
      const qty = safeQuantity(item._id);
  ...
  const increment = async (id) => {
    const newQuantity = safeQuantity(id) + 1;
    setQuantity((prev) => ({
      ...prev,
      [id]: newQuantity,
    }));
  ...
  const decrement = async (id) => {
    const currentQty = safeQuantity(id);
  ...
  const removeCartItem = async (index, id) => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('cartUpdated', { detail: { action: 'remove', productId: id } }));
    } catch (err) {
      if (isMounted.current) {
        ...
        {data.map((item, index) => (
          ...
          >
            ...
            <div className="flex-1">
              ...
              <h3 className="text-gray-600">Rating: {item.rating || 4.5}</h3>
              <h3 className="text-gray-600">Count: {item.count || 9}</h3>
              {/* Quantity Controls */}
              <div className="flex items-center space-x-4 mt-3">
                ...
                <div className="flex items-center space-x-3 bg-gray-50 rounded-lg p-1">
                  <button
                    ...
                    disabled={loading || safeQuantity(item._id) <= 1}
              ...
              <div className="mt-3 text-lg font-semibold text-gray-800">
                Subtotal: ₹{(safeQuantity(item._id) * Number(item.Discountedprice || 0)).toFixed(2)}
              </div>
            </div>
            {/* Remove Button */}
