...
function Home() {
  ...
  const ProductGrid = (
    <div className="grid gap-6 px-4 sm:px-6 lg:px-8 py-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 max-w-[1400px] mx-auto">
      {filteredProducts.length > 0 ? (
        filteredProducts.map((item) => (
          <div
            key={item._id}
            className="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-2 border border-gray-100 relative"
          >
           
            <Link
              to={`/product/${item._id}`}
              className="block relative bg-gradient-to-br from-gray-50 to-gray-100"
            >
              <div className="relative h-[240px] flex justify-center items-center p-6">
                <img
              ...
              {item.Originalprice &&
                ...
                item.Originalprice > item.Discountedprice && (
                  <div className="absolute top-3 right-3 bg-gradient-to-r from-red-500 to-pink-500 text-white text-xs font-bold px-3 py-1 rounded-full shadow-md">
                    {Math.round(
                      ((item.Originalprice - item.Discountedprice) /
                        item.Originalprice) *
                        100
            ...
            <div className="p-5 space-y-3">
              ...
              <Link
                to={`/product/${item._id}`}
                className="block w-full bg-gradient-to-r from-blue-600 to-purple-600 text-white text-center py-3 rounded-xl font-semibold hover:from-blue-700 hover:to-purple-700 transform hover:scale-[1.02] transition-all duration-300 shadow-md mt-4"
              >
                View Details
              </Link>
            </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-purple-50 pt-[80px]">
      {/*Added pt-[80px] to prevent header overlap */}
      {user?.role === "admin" ? (
        //  Admin View
        <div className="py-8">
        ...
        <>
          ...
          {/*  Featured Header */}
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-4 text-center">
